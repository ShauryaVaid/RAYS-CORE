...
This skill is separate from the RAYS Email gateway adapter. The gateway
adapter lets people email the agent and uses RAYS' built-in IMAP/SMTP
...
To reply non-interactively from RAYS, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from RAYS)** — pipe the message via stdin:
