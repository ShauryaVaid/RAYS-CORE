...
short to a music video to an ASCII loop — in a RAYS Kanban pipeline that
...
3. **Generates** a setup script that creates RAYS profiles, project workspace, and the initial kanban task
...
For mapping role → which RAYS skills + toolsets it loads, see
...
3. Creates each RAYS profile via `hermes profile create --clone`
