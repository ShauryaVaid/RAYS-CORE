#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `rays kanban list` and
...
def rays_available() -> bool:
    return shutil.which("rays") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed rays CLI."""
    try:
        out = subprocess.run(
            ["rays", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `rays kanban list`
    out = subprocess.run(
        ["rays", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not rays_available():
        print("ERROR: 'rays' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
