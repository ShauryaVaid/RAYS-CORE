...
For long-lived processes: RAYS gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside RAYS you have two practical options:
...
## Debugging RAYS-specific Processes
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For RAYS subagents, debug one process at a time.
