...
All per-subject data lives under PDD_DATA_DIR (default: $RAYS_HOME/unbroker),
which is the same trust boundary Rays uses for .env and OAuth tokens.
"""
...
def rays_home() -> Path:
    return Path(os.environ.get("RAYS_HOME") or (Path.home() / ".rays"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else rays_home() / "unbroker"
