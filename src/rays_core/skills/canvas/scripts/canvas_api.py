#!/usr/bin/env python3
"""Canvas LMS API CLI for Rays Agent.
...
def _check_config():
    ...
    if missing:
        rays_env = os.path.join(
            os.environ.get("RAYS_HOME", os.path.expanduser("~/.rays")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {rays_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Rays Agent"
    )
