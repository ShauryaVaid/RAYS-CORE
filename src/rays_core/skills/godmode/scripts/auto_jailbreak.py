...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("RAYS_HOME", os.path.expanduser("~/.rays")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("RAYS_HOME", Path.home() / ".rays")) / "skills" / "red-teaming" / "godmode"
...
RAYS_HOME = Path(os.getenv("RAYS_HOME", Path.home() / ".rays"))
CONFIG_PATH = RAYS_HOME / "config.yaml"
PREFILL_PATH = RAYS_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Rays models — already uncensored, just needs clean prompt
    "rays": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "rays" in model_lower or "nous" in model_lower:
        return "rays"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Rays config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.rays/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Rays for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Rays for changes to take effect.")
