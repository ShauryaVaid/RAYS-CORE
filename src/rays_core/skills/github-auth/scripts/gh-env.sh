#!/usr/bin/env bash
# GitHub environment detection helper for Rays Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _rays_env="${RAYS_HOME:-$HOME/.rays}/.env"; [ -f "$_rays_env" ] && grep -q "^GITHUB_TOKEN=" "$_rays_env" 2>/dev/null; then
    GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_rays_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
