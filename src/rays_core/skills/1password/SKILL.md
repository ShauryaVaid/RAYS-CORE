...
- `tmux` available for stable authenticated sessions during RAYS terminal calls (desktop app flow only)
...
### Service Account (recommended for RAYS)
...
## RAYS Execution Pattern (desktop app flow)

RAYS terminal commands are non-interactive by default and can lose auth context between calls.
