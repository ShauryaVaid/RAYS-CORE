...
# Authoring RAYS-Agent Skills (in-repo)
...
2. **Choose the right context load.** A model-invoked RAYS skill pays for its description every turn. Keep descriptions focused on trigger classes and the skill's distinctive behavior. Put details in the body or linked references.
...
- RAYS-specific recipes (tests via scripts/run_tests.sh, ui-tui paths, etc.)
...
3. **Description too generic or trigger buried past char 57.** The system prompt
   ...
   - Good: `Use when debugging RAYS skill discovery failures.`
   - Bad: `This skill contains detailed guidance for agents working on RAYS skill discovery failures.`
