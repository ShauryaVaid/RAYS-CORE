...
Usage (via cron with --no-agent):
    ...
    rays cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$RAYS_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Rays-Watcher/1.0"})
