...
def _state_dir() -> Path:
    ...
    # Default: $RAYS_HOME/watcher-state/, falling back to ~/.rays/watcher-state/.
    rays_home = os.environ.get("RAYS_HOME") or str(Path.home() / ".rays")
    return Path(rays_home) / "watcher-state"
