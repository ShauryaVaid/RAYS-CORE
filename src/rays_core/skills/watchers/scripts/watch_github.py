...
Usage (via cron with --no-agent):
    ...
    rays cron create rays-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$RAYS_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name rays-issues --repo NousResearch/rays-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Rays .env file
(``${RAYS_HOME:-~/.rays}/.env``) to avoid the 60 req/hr
