...
RAYS_HOME_DIR="${RAYS_HOME:-$HOME/.rays}"
RAYS_CFG="${RAYS_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Rays config has twozero_td MCP entry ──
if [[ ! -f "$RAYS_CFG" ]]; then
    echo -e " ${FAIL} Rays config not found at ${RAYS_CFG}"
    manual_steps+=("Create ${RAYS_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$RAYS_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Rays config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Rays config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${RAYS_CFG} manually"); }
    manual_steps+=("Restart Rays session to pick up config change")
fi
