...
Usage:
  ...
  <server_name> matches the files in $RAYS_HOME/mcp-tokens/<server>.json etc.
...
def _rays_home():
    # Prefer Rays' own resolver (profile-safe); fall back to env then ~/.rays.
    try:
        from rays_constants import get_rays_home
        return str(get_rays_home())
    except Exception:
        return os.environ.get("RAYS_HOME") or os.path.expanduser("~/.rays")


def _tokens_dir():
    return os.path.join(_rays_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "rays-diag", "version": "1.0"}},
        },
        headers={
