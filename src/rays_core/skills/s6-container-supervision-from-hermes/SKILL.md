...
# RAYS s6-overlay Container Supervision

## When to use this skill

Load this skill when you're working on:
- Adding or removing a static service in the RAYS Docker image (something that should be supervised at every container start, like the dashboard)
...
If you're just running the RAYS Agent and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
- `hermes-tool-quirks`: Specific RAYS-tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with hermes built-in tools.
