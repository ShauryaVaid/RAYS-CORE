...
RAYS drives [cua-driver](https://github.com/trycua/cua) under the hood
for the platform plumbing. The RAYS-side `computer_use` tool exposed
in this skill is a higher-level RAYS vocabulary; the raw cua-driver
...
RAYS intentionally keeps THIS skill focused on the RAYS-side
...
When `cua-driver skills install` autodetects RAYS (planned follow-up
