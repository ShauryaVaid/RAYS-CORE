#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Rays Agent.
...
# Ensure sibling modules (_rays_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _rays_home import display_rays_home, get_rays_home

RAYS_HOME = get_rays_home()
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Rays profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Rays Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Rays Docker image's "
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Rays home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_rays_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Rays")
