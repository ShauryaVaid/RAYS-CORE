...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the RAYS terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For RAYS itself, `model.provider: openai-codex` uses RAYS-managed Codex
...
When invoking the Codex CLI from a RAYS gateway/service context (for example,
