...
RAYS Agent is an open-source AI agent framework by Nous Research that runs in your terminal, a native desktop app, messaging platforms, and IDEs. It's in the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. RAYS works with any LLM provider (OpenRouter, Anthropic, OpenAI, Google, DeepSeek, xAI, local models, and 20+ others) and runs on Linux, macOS, Windows, and WSL.

What makes RAYS different:

- **Self-improving through skills** — RAYS learns from experience by saving reusable procedures as skills that load into future sessions.
...
- **Profiles** — run multiple independent RAYS instances with isolated configs, sessions, skills, and memory.
...
## Spawning Additional RAYS Instances

Run additional RAYS processes as fully independent subprocesses — separate sessions, tools, and environments.
...
RAYS uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
- **"delegate_task is capped at N" reports** — see `references/delegate-task-concurrency-diagnosis.md`. Three real cap paths in RAYS; if none fired, the model is self-limiting and rationalising it as "the runtime caps."
