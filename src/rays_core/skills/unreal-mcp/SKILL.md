...
Companion skill for the `unreal-engine` entry in the RAYS MCP catalog. The
...
Two halves, in this order: the editor side must be up before RAYS connects.
...
Do NOT use `ModelContextProtocol.GenerateClientConfig` for RAYS — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. RAYS connects
...
2. Start the RAYS session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through RAYS
they appear as:
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — RAYS runs batched calls
...
- **Start order matters.** Editor + server up first, then the RAYS
...
- **Timeouts on long operations.** RAYS' per-call default is 120 s; asset
